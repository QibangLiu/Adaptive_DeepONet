...
def GetL2Error(filebase):
    ...
    for iter in iters_path:
        if os.path.exists(os.path.join(iter, "TestL2Error.csv")):
            trainDataIDX=np.genfromtxt(os.path.join(iter, "trainDataIDX.csv"), dtype=int, delimiter=",")
            num_samples.append(trainDataIDX.shape[0])
            L2error=np.genfromtxt(os.path.join(iter, "TestL2Error.csv"), dtype='float32', delimiter=",")
...
for para in paras:
    if para[0]=='k-mean':
        project_name = (
            "adaptive_kmean" + "dN" + para[1]+'case1'
        )
    else:
        project_name = (
        "adapt_tl_PDF_k" + para[0] + "c" + para[1] + "dN" + para[2]+'case0'
        )
    filebases.append(os.path.join(prefix_filebase, project_name))

# filebase1='/scratch/bblv/qibang/repository/Adap_data_driven_possion/saved_model/adaptive_k0c100dN200'
# filebase2='/scratch/bblv/qibang/repository/Adap_data_driven_possion/saved_model/adaptive_k50c100dN200'
# filebase3='/scratch/bblv/qibang/repository/Adap_data_driven_possion/saved_model/adaptive_k200c100dN200'
# filebase4='/scratch/bblv/qibang/repository/Adap_data_driven_possion/saved_model/adaptive_k200c0dN200'

# filebases=[filebase1,filebase2,filebase3,filebase4]
